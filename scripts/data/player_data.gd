...
func deserialize(data: Dictionary) -> void:
	level = clampi(int(data.get("level", 1)), 1, BalanceConfig.MAX_LEVEL)
	exp = maxi(0, int(data.get("exp", 0)))
	gold = maxi(0, int(data.get("gold", 0)))
	bonus_attack = clampi(int(data.get("bonus_attack", 0)), 0, BalanceConfig.MAX_BONUS_ATTACK)
	bonus_defense = clampi(int(data.get("bonus_defense", 0)), 0, BalanceConfig.MAX_BONUS_DEFENSE)
	total_kills = maxi(0, int(data.get("total_kills", 0)))
	total_gold_earned = maxi(0, int(data.get("total_gold_earned", 0)))
	total_damage_dealt = maxi(0, int(data.get("total_damage_dealt", 0)))
	total_damage_taken = maxi(0, int(data.get("total_damage_taken", 0)))
	death_count = maxi(0, int(data.get("death_count", 0)))
	bosses_defeated = maxi(0, int(data.get("bosses_defeated", 0)))
	highest_stage = maxi(1, int(data.get("highest_stage", 1)))
	play_time_seconds = maxf(0.0, float(data.get("play_time_seconds", 0.0)))
	recalc_stats(false)
	hp = clampi(int(data.get("hp", max_hp)), 1, max_hp)
	exp = mini(exp, exp_to_next)
